    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                elif type(self.ant) == QueenAnt and self.ant.imposter == False :
                    return None
                else:
                    self.ant = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if type(self.ant.ant) == QueenAnt and self.ant.ant.imposter ==  False:
                        return None
                    else:
                        self.ant.ant = None
                else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        if self.place.ant is None or type(self.place.ant) == NinjaAnt:
            return False
        return True
        # END Problem 6A
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        while place != hive:
            if place.bees and self.pred(place_range):
...
class LongThrower(ThrowerAnt):
    ...
    food_cost = 2
    min_range = 5
    pred  = lambda self, x: x >= self.min_range
    implemented = True   # Change to True to view in the GUI
    # END Problem 4B
...
class ShortThrower(ThrowerAnt):
    ...
    food_cost = 2
    max_range = 3
    pred = lambda self, x: x<= self.max_range    
    implemented = True   # Change to True to view in the GUI
    # END Problem 4B
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            ...
            if random_or_none(self.place.bees):
                bee_eaten = random_or_none(self.place.bees)
            ...
            while place.exit:
                if place.ant:
                    if place.ant.container:
                        if place.ant.ant and not place.ant.ant in self.ants_doubled:
                            place.ant.ant.damage *= 2
                            self.ants_doubled += [place.ant.ant]
                    if not place.ant in self.ants_doubled:
                        place.ant.damage *= 2
                        self.ants_doubled += [place.ant]
                place = place.exit
