...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    num_rolled, flag, result = 0, False, 0
    while num_rolled < num_rolls:
        result_this_turn = dice()
        num_rolled += 1
        if result_this_turn == 1:
            flag = True
        result += result_this_turn
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    max_ave, num_maximizing_ave = -1, -1
    for num_rolls in range (1,11):
        ave = make_averaged(roll_dice, num_samples)(num_rolls, dice)
        if ave > max_ave:
            max_ave = ave
            num_maximizing_ave = num_rolls
