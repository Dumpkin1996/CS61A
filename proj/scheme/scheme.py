...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(scheme_eval_env(expr.first), expr.second.map(scheme_eval_env), env)
        # END PROBLEM 5
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        result = scheme_eval(expressions.second.first, env, True)
        return result
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            def scheme_eval_env(expr):
                return scheme_eval(expr, env)
            result = scheme_apply(scheme_eval_env(first), rest.map(scheme_eval_env), env)
            # END Extra Credit
    return result
