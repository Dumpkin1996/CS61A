    ...
    i_love_music
      ...
      trance
        darude
          sandstorm
    """
    child1 = tree('pop', [tree('justin bieber',[tree('single',[tree('what do you mean?')])]), tree('2015 pop mashup')])
    child2 = tree('trance',[tree('darude',[tree('sandstorm')])])
    return tree(username, [child1, child2])
...
def num_songs(t):
    ...
    if is_leaf(t):
        ...
        return sum([num_songs(c) for c in children(t)])
...
def add_song(t, song, category):
    ...
    if entry(t) == category:
        ...
        return tree(entry(t), [add_song(c, song, category) for c in children(t)])
...
def delete(t, target):
    ...
    kpop_king
      korean
        ...
        wedding dress
    """
    if children(t) == [] and entry(t) != target:
        return t
    else:
        return tree(entry(t), [delete(c, target) for c in children(t) if entry(c) != target])
