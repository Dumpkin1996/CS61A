...
def has_cycle(link):
    ...
    list_of_links = [link]
    while link.rest:
    	if link.rest in list_of_links:
    		return True
    	else:
    		list_of_links += [link.rest]
    		link = link.rest
    return False
...
def has_cycle_constant(link):
    ...
    """
    slow_tracker = link
    fast_tracker = link.rest
    while not fast_tracker == Link.empty and not fast_tracker.rest == Link.empty:
        if slow_tracker == fast_tracker:
            return True
        else:
            slow_tracker = slow_tracker.rest
            fast_tracker = fast_tracker.rest.rest
    return False
...
def is_bst(t):
    ...
    if Tree.is_leaf(t):
    	...
    	if len(t.children) > 2:
    		...
    		return is_bst(t.children[0]) and is_bst(t.children[1]) and bst_min(t.children[1]) >= t.entry and bst_max(t.children[0]) <= t.entry
