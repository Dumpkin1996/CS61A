...
(define (cond-to-if expr)
  (if (equal? (predicate (first-clause expr)) 'else) (consequent (first-clause expr))
  	(list 'if (predicate (first-clause expr)) (consequent (first-clause expr)) (cond-to-if (cons 'cond (rest-clauses expr))) )
  )
)
